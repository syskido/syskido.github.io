...
필요한 기능이 존재하지 않는 경우, 커스텀 플러그인을 작성할 수 있습니다. Jekyll 프로젝트의 플러그 폴더에 Ruby 파일을 추가하면 됩니다.

예를 들어, 간단한 필터 플러그인을 작성해보겠습니다. 플러그그 폴더에 `reverse_filter.rb` 파일을 추가하고 다음 코드를 작성합니다.
...
Jekyll 플러그인은 주로 다음 세 가지 형태로 작성할 수 있습니다:
...
필터 플러그인은 Liquid 템플릿 언어에서 데이터를 변형할 때 사용됩니다.  폴더에 새로운 Ruby 파일을 생성하고 필터를 정의할 수 있습니다.



# 태그 플러그인 작성

태그 플러그인은 Liquid 템플릿 언어에서 새로운 태그를 정의할 때 사용됩니다.  폴더에 새로운 Ruby 파일을 생성하고 태그를 정의할 수 있습니다.

## 예시: 현재 날짜를 출력하는 태그

1.  폴더에 `current_date_tag.rb` 파일을 생성합니다.
2. 다음 코드를 작성합니다.
...
제너레이터 플러그인은 Jekyll 사이트를 빌드할 때 새로운 페이지를 생성하거나 기존 페이지를 변형합니다.  폴더에 새로운 Ruby 파일을 생성하고 제너레이터를 정의할 수 있습니다.

## 예시: 사이트에 페이지 목록 생성

1.  폴더에 `page_list_generator.rb` 파일을 생성합니다.
2. 다음 코드를 작성합니다.
